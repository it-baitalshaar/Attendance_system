...
| `departments` | `id` | `name`, `theme_id`, `allow_future_attendance`, OT toggles |
...
| `departmentService` | Departments CRUD + employee counts |
...
| File | Tool |
|------|------|
| `.cursor/rules/project-index.mdc` | Cursor (`alwaysApply: true`) |
...
## 20. Maintenance — keep this file current

When you change the project, update **only the affected section** plus one changelog row:

```
- New API route     → §5 + §8
- New table/column  → §6 + §11
- New admin tab     → §4 + §9
- New env var       → §2
- Payroll rule change → §12 + note in changelog
- New migration     → §11 one-line summary
```

**Token tip for AI:** Load `docs/PROJECT_INDEX.md` + the 1–3 files from §0 quick nav — avoid reading whole `admin/page.tsx` or `redux/slice.ts` unless editing them.
