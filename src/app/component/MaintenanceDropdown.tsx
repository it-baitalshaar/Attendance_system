...
//           <label className="block text-white mb-2 text-xl items-end">مشاريع الصيانة</label>
//           <select 
//             value={project} 
//             onChange={(e) => setProject(e.target.value)} 
//             className="w-[7.8rem] p-2 border rounded-lg focus:ring focus:ring-blue-200 text-black items-end"
//           >
//             <option value="" disabled>مشاريع الصيانه</option>
//             <option value="Project A">مشروع </option>
//             <option value="Project B">مشروع </option>
//             <option value="Project C">مشروع </option>
//           </select>
//         </div>
//     </>
//   );
...
const MaintenanceDropdown = ({employee_id, input_index, departments}:MaintenanceDropdownProps) => {
  const [project, setProject] = useState('');
  ...
  let [overtime, setOvertime] = useState<number[] | number>(Array.from({ length: 5 }, (_, i) => i));
...
//   if (the_employee?.employee_status![0].status_attendance === 'present')
//   {
//     showHours(the_employee?.employee_status![0].status_employee)
//   }
// }, [the_employee?.employee_status![0].status_employee])

// if (employees_statis)
  useEffect(() => {
    async function getProjects() {
      const data = await fetchProjects('Maintenance');
  ...
  useEffect(() => {
    ...
    // {
    //   console.log("insdie first condition ", the_employee?.employee_status![0].status_employee )
    //   if (the_employee?.employee_status![0].status_employee !== 'Present')
    //   {
    //     console.log("inside the anohter condition ")
    //     setOvertimeInput(true)
    //   }
...
const handleOvertTimeHoursChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  return (
    <div className="mb-4 flex flex-col items-end mt-8">
      ...
      >
        <option  value="" disabled>مشاريع الصيانة</option>
...
export default React.memo(MaintenanceDropdown);
