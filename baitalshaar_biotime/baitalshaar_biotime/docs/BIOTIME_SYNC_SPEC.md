...
### API
- **Endpoint:** `GET {BIOTIME_BASE_URL}/att/api/transactionReport/` (default). Your device or API doc may use a different path (e.g. `GET .../iclock/api/transactions/`). Set **`BIOTIME_TRANSACTION_PATH`** (or **`BIOTIME_TRANSACTIONS_PATH`**) in `.env` to match (e.g. `BIOTIME_TRANSACTION_PATH=/iclock/api/transactions/`).
- **Example base URL:** `http://192.168.1.50:80` or `http://localhost:8080` (must be reachable from the machine running the script).
...
1. **Validate:** `employee_code` and `punch_time` present; `punch_state` in `{0, 1}`. Else skip (count as **invalid**).
...
## 7. Sync stats: what “invalid” means

The script counts transactions as **invalid** when:

1. **Missing or bad fields:** `employee_code` or `punch_time` is missing/empty, **or**
2. **Unsupported punch_state:** `punch_state` is not `0` (check-in) or `1` (check-out). Some BioTime devices or APIs send other values (e.g. `2` = break, `3` = overtime). We only sync 0 and 1, so those are skipped as invalid.  
   **Fix (if you need them):** Extend the script to map other values to checkin/checkout or to a separate action, or ignore and accept the invalid count.
3. **Unparseable date:** `punch_time` could not be converted to a valid date (YYYY-MM-DD). For example an unexpected string format.  
   **Fix:** Set `BIOTIME_DEBUG_INVALID=1` and run sync; the console will log sample invalid transactions and the reason so you can adjust parsing or field names.
